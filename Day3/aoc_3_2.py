...
for i in range (0, len(ox_lines[0])-1):
	...
	if one_occ >= zero_occ:
		ox_list = [line for line in ox_lines if line[i] == '1']
	else:
		ox_list = [line for line in ox_lines if line[i] == '0']
	ox_lines = ox_list
...
for i in range (0, len(co2_lines[0])-1):
	...
	if zero_occ <= one_occ:
		co2_list = [line for line in co2_lines if line[i] == '0']
	else:
		co2_list = [line for line in co2_lines if line[i] == '1']
	co2_lines = co2_list
	# check if a single value is reached
	if len(co2_list) == 1:
		break
